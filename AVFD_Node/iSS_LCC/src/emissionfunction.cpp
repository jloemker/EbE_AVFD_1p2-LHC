...
*/
    ...
    for (int n = 0; n < number_of_chosen_particles; n++) {
        ...
        // buffers are used to speed up the output process
        char line_buffer[500];            // only used in text mode
        std::stringstream sample_str_buffer;   // to speed up outputing
        std::stringstream control_str_buffer;
        ...
        double AVFD_LCC_Ratio = 0.01*local_charge_conservation;
        for (long repeated_sampling_idx = 1; 
            ...
            long sample_idx = 1;
            while (sample_idx <= number_to_sample) {
                // skip some of the "anti-particles"
                if ((local_charge_conservation>0) 
                    && ((charge<0)||((charge==0)&&(baryon<0))||
                    ((charge==0)&&(baryon==0)&&(strange<0)))
                    && (AVFD_LCC_Ratio > 
                ...
                if ((local_charge_conservation>0)&&
                    ((charge>0)||((charge==0)&&(baryon>0))||
                        ((charge==0)&&(baryon==0)&&(strange>0)))&&
                    (AVFD_LCC_Ratio > ran_gen_ptr.lock()->rand_uniform())
                    )
                {
                    // AVFD
                    double avfd_T=surf->Tdec;
                    double avfd_muB=surf->muB;
                    double avfd_muC=surf->muC;
                    double avfd_muS=surf->muS;
                    double avfd_mu=avfd_muB*baryon+avfd_muC*charge+avfd_muS*strange;
                    double avfd_ratio = exp(-2.*avfd_mu/avfd_T);
                    int avfd_ratio_int = floor(avfd_ratio);
                    double avfd_ratio_frac = avfd_ratio - avfd_ratio_int;
                    if (avfd_ratio_frac > ran_gen_ptr.lock()->rand_uniform())
                    {avfd_ratio_int+=1;} // add a negative particle
                    //avfd_ratio_int=1;//this would ensure equal number
                    for(int avfd_i=0; avfd_i<avfd_ratio_int; avfd_i++)
                    {
                        double pT2, phi2, y_minus_eta_s2;
                        int status2 = 0;
                        std::string particle_string2;
                        ...
                        particle_string2 = add_one_sampled_particle(
                                    repeated_sampling_idx, FO_idx, surf,
