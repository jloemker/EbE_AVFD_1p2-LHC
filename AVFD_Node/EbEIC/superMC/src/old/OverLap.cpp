...
OverLap::OverLap(ParameterReader* paraRdr_in, int a, double signnin, int deformed_in, int z_proton)
{
  paraRdr = paraRdr_in;
  ...
  else if(a==96&&z_proton==44){
    //Reparametrization (Ru)
    rad = 5.085;
    dr = 0.46;
    density0 = 0.16;
  }
  else if(a==96&&z_proton==40){
    //Reparametrization (Zr)
    rad = 5.02;
    dr = 0.46;
    density0 = 0.16;
  }
  ...
  if(deformed){
    ...
    if(atomic == 197){
      ...
      beta2 = 0.28;
      beta4 = 0.093;
    }else if(atomic == 96&&z_proton==44){
      //(Ru)
      beta2 = 0.158;
      beta4 = 0.00;
    }else if(atomic == 96&&z_proton==40){
      //(Zr)
      beta2 = 0.08;
      beta4 = 0.00;
    }else{
...
// Shuzhe Shi modidied this function to enable testing the sensitivity of nucleon distribution.
void OverLap::getDeformRandomWS(double& x, double& y, double& z, int eCharge)
{
  double rad1 = rad;
  double rwMax1 = rwMax;
  double r = 0.0;
  double cx = 1.0; // cx is cos(theta)
  double RhoNucleon; // Shuzhe Shi Added. Nucleon Density Distribution.
  //AVFD:DISTRIBUTION_TEST
  #ifdef DISTRIBUTION_TEST
    deformed = DT_DEFORM;
    #if DT_WS==1
      rad=DT_R; dr=DT_a; beta2=DT_beta2; beta4=D_beta4;
      rwMax = 1.0/(1.0+exp(-rad/dr));
    #endif
    rmaxCut = 10.0;
  #endif
  if (deformed) // Deformed
  {
    do {
  ...
  else // Spheric
  {
    do {
      ...
      r = rmaxCut*pow(drand48(),1.0/3.0);
        ...
        // Woods-Saxon
        RhoNucleon = 1.0/(1.0+exp((r-rad)/dr));
      #else
      #if DT_WS==1 
      // Test, Woods-Saxon
      {
      ...
      #else
      {
        double R, a, rho, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, expcore;
        if(eCharge==0)//n
        {
          R  = DT_DF_n_R;
          a  = DT_DF_n_a;
        ...
        expcore = exp((r-R)/a);
        RhoNucleon = c1*(1+r*r*(c2+r*(c3+r*(c4+r*(c5+r*(c6+r*(c7+r*(c8+r*(c9+r*(c10+r*(c11+r*c12)))))))))))/(1+expcore*expcore) + rho/(1+expcore);
        rwMax = 1.1;
      }
      #endif
      #endif
    } while(drand48()*rwMax > RhoNucleon);
    // Shi Note: Assign Woods-Saxon Distribution here.
    cx = 1.0-2.0*drand48();
    double sx = sqrt(1.0-cx*cx); // sx is sin(theta)
    double phi = 2*M_PI*drand48();
    x = r*sx*cos(phi);
